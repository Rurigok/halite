 ...
 * Version: v3 (Valiant)
 *
 * Look-ahead border search target designation with resource partitioning.
 *
 * @author Andrew Sanetra (rurigok)
 */
public class Valiant {

	public static final String BOT_NAME = "Valiant";
    ...
    public static void main(String[] args) throws java.io.IOException {
        ...
        while (true) {
            ...
            // Acquire target nodes
			ArrayList<Location> targetList = acquireTargetList();
	...
	public static ArrayList<Location> acquireTargetList() {
		HashSet<Location> targetSet = new HashSet<>();
		Site mySite;
		Site targetSite;
		...
		for (int y = 0; y < gameMap.height; y++) {
			for (int x = 0; x < gameMap.width; x++) {
				...
				if (mySite.owner == myID) {
					for (Direction d : Direction.CARDINALS) {
						targetLocation = getLocationInDirection(x, y, d);
						targetSite = gameMap.getSite(targetLocation);
						if (targetSite.owner != myID) {
							if (!targetSet.contains(targetLocation)) {
								targetSet.add(targetLocation);
							}
						}
					}
				}
		...
		// TODO; upgrade target designation; currently chooses random targets.
		ArrayList<Location> finalTargetList = new ArrayList<>(targetSet);
		Collections.shuffle(finalTargetList);
		int numToKeep = (int) (Math.random() * targetSet.size());
		finalTargetList = finalTargetList.subList(0, Math.min(finalTargetList.size(), numToKeep));
